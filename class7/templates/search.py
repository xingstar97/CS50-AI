...
def find_templates(examples, corpus):
    ...
    # middle here is tuple(middle, True/False)
...
def match_query(q1, q2, order, corpus):
    ...
    # format string {q1}: amazon, {{0, 10}}: {0, 10}
    # ?! Negative Lookahead: q1 is not followed by q1
...
def common_prefix(*s):
    ...
    return "".join(
        ...
        # takewhile: This allows considering an item from the iterable 
        # until the specified predicate becomes false for the first time.
        # it returns a list
        # ch is a tuple, which includes identical items
    )
...
# [::2] means choose every other elements in the list
# return common_prefix(*s)[::-1] because the results returned by common_prefix needs to be reversed
