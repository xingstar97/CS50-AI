...
class NimAI():
    ...
    def best_future_reward(self, state):
        ...
        for action in actions:
            try:
                if self.q[state, action] > future_rewards:
                    future_rewards = self.q[state, action]
            except KeyError:
    ...
    def choose_action(self, state, epsilon=True):
        ...
        for action in actions:
            try:
                # if self.q[state, action] == -2:
                #     best_actions.add(action)
                if self.q[state, action] > rewards:
                    best_actions = {action}
                    rewards = self.q[state, action]
                    # update rewards!!!!!!!!
            except KeyError:
                self.q[state, action] = 0
        if len(best_actions) == 0:
            best_actions = actions
        options = [best_actions, actions]
        if not epsilon:
